            ...
            <div class="card">
              <div class="card-header pl-3 pt-2 pb-2">
                ...
                >
                  <router-link
                    to="/loans/create"
                    class="btn btn-success text-uppercase mr-1 btn-xs"
                    ><i class="fas fa-plus-circle"></i> NOvo
                    Empréstimo</router-link
                  >
                </div>
              </div>
              <!-- /.card-header -->
...
export default {
  ...
  methods: {
    ...
    monthly_fees_recognize(status) {
      this.overlay = true;
      Axios.post(
        `/loans/monthly_recognization`,
        { headers: { Authorization: `Bearer ${token.state.token}` } },
        {
          loans: this.checkedLoans,
